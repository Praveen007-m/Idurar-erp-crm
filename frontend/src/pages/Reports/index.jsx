...
export default function Reports() {
  ...
  // ── FIX: API returns { success, result: { collections, statusBreakdown, planWise } }
  ...
  const formatStatus = (v) => {
    ...
    const colors = {
      Paid: '#52c41a', Late: '#fa8c16', Partial: '#faad14',
      Default: '#ff4d4f', 'Not Started': '#8c8c8c',
    };
  ...
  const statusColumns = [
    ...
    { title: translate('Percentage'), dataIndex: 'percentage', key: 'percentage', align: 'right', render: (v) => `${v}%` },
  ];

  // ── FIX: planWise uses planGroup + customers + collected + pending fields
  const planColumns = [
    {
    ...
    },
    {
      ...
      key:       'collected',
      align:     'right',
      render:    (v) => <span style={{ color: '#52c41a' }}>{moneyFormatter({ amount: v ?? 0 })}</span>,
    },
    {
      ...
      key:       'pending',
      align:     'right',
      render:    (v) => <span style={{ color: '#ff4d4f' }}>{moneyFormatter({ amount: v ?? 0 })}</span>,
    },
  ];
    ...
    <DashboardLayout>
      <div style={{ padding: isMobile ? '14px 10px' : '24px' }}>
          ...
          {statCards.map((card, idx) => (
            <Col key={idx} xs={12} sm={12} md={6}>
              <Card size="small" bordered={false}
                ...
                <Statistic
                  title={<span style={{ fontSize: isMobile ? 11 : 13, color: '#8c8c8c' }}>{card.title}</span>}
                  value={card.value}
                  prefix={card.icon}
        ...
        <Row gutter={[12, 12]}>
          <Col xs={24} md={12}>
            <Card
              title={<Space><PieChartOutlined /><span style={{ fontSize: isMobile ? 13 : 15 }}>{translate('Status Breakdown')}</span></Space>}
          ...
          <Col xs={24} md={12}>
            <Card
              title={<Space><PieChartOutlined /><span style={{ fontSize: isMobile ? 13 : 15 }}>{translate('Plan-wise Analytics')}</span></Space>}
              bordered={false} size="small"
              style={{ borderRadius: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <Table
